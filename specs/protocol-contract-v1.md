...
| Multi-node orchestration test | n/a | n/a | `core/tests/mesh_sim.rs` now calls production `mesh_receive` (its receive third-copy is deleted, D0); it still reimplements meet | delete | production `mesh_receive` landed (D0); `mesh_meet` absorbs the remaining meet half in D2 |
