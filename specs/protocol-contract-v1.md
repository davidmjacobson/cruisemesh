...
The rule is enforced structurally first: chunk transfer is reachable from no
dispatch, carry, spray, or framing path, and none of it is exported over
UniFFI, so there is no code path by which a blob byte could reach an envelope.
That is a strong property while the module is dark and a weak one afterwards,
so it is no longer the only thing holding the rule up. The adversarial half is
owned and executable now: a simulation sends media continuously through real
identities, real sealing, real carry queues and the real spray policy, then
scans every byte it ever framed for the blob's ciphertext; the spray policy
charges the largest manifest the codec will emit as an ordinary envelope and
shows that no blob granularity fits an encounter budget at all; and the BLE
...
Both halves are owned in core now. The consent half is a composition rather
than a second policy: the blob verdict consults the same
`core_relay_network_permitted` the mailbox does, so a roaming path the relay
refuses is a path a blob cannot take either, and a confirmation cannot buy
past it — the only override is the Advanced roaming toggle already inside that
verdict. A LAN source never consults it at all, because a roaming SIM says
nothing about a cabin Wi-Fi.

The rule cuts both ways, and the whole-matrix assertion states both
directions. A constrained path (Data Saver, Low Data Mode) is an *expensive*
path, not a closed one — the mailbox keeps syncing on it and only its
unattended uploads wait — so a blob on it is offered with its size and started
on a tap. Nothing ever clears that verdict, so deferring instead would leave a
photo unreachable for as long as the setting is on, with no offer on the bubble
to escape through. Being unable to spend at all is a defect the same way
spending unasked is.

What is *not* claimed here is a relay transfer that works: no
relay blob store exists yet (see `BLOB-06`), so the expensive path this
verdict describes is the one phase 4 will open. The rule is pinned before the
path exists rather than after, which is the order that keeps it.
