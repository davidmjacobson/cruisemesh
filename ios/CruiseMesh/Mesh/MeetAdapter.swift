...
/// returned them.
///
/// What core decides here, and this type does not: whether this link owes a
...
/// re-derived in Swift beside a second copy in Kotlin and a third in the
/// simulator; it now lives once in `core/src/session/mesh_meet.rs`.
///
/// What this type keeps is what a driver is for: the transports, the send
...
/// measured on `SprayPolicy.nowMs` — a monotonic clock — because an NTP
/// correction landing mid-session must not expire a spray window early and buy
/// the burst the window exists to prevent. The planner takes both and hands
/// each to the state that counts with it; this adapter is where the two are
/// read, once per encounter, so no lane can see them drift apart mid-plan.
///
...
struct MeetAdapter {
    ...
    typealias Send = (String, Data) -> Void
    ...
    var sprayNow: () -> Int64 = { SprayPolicy.nowMs }

    /// Runs one encounter through the core planner and sends what it returns.
    ///
    /// Returns the planner's bounded work counts, or `nil` when the plan
    ...
    func encounter(
        ...
        peerAuthenticated: Bool = false,
        peerCapabilities: UInt32? = nil
    ) -> CoreMeetWork? {
        let outcome: CoreMeetOutcome
        do {
            outcome = try store.corePlanMeshMeet(
                router: router,
                spray: spray,
                offers: offers,
        ...
        // Digest, then targeted drain, then spray — the field order in the
        // order the planner returned it.
        //
        // It is load-bearing, not cosmetic: core's exchange window opens when
        // the digest is enqueued, and on a slow BLE link a multi-KB drain
        // queued ahead of it holds it in the FIFO past the window, so the
        // peer's answer arrives to a shut gate. Iterating the three lists in
        // field order is what makes that ordering something the planner
        // enforces instead of a convention every call site had to remember.
        for frame in outcome.digestFrames { send(address, frame) }
        for frame in outcome.targetedFrames { send(address, frame) }
        for frame in outcome.sprayFrames { send(address, frame) }
        return outcome.work
    }
}
