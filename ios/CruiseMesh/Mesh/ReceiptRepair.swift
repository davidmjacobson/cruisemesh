...
enum ReceiptRepair {
    ...
    /// `handleDigest`'s backfill re-seals it if that peer later reports the gap
    /// in its own gap-aware digest. An over-report therefore costs at worst a
    /// slightly early delivered tick, never a message the sender still owes.
