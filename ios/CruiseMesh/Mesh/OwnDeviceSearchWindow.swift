...
/// long it waits. Without a motive of its own, Bonjour is the only channel
/// between two phones of one person — and one stale Bonjour record is the whole
/// of the field failure this exists to prevent.
///
...
/// the two facts it compares against — the roster last observed and the
/// shortfall last observed.
///
/// Mirrors Android's `OwnDeviceSearchWindow`. Callers hold `meshQueue`.
final class OwnDeviceSearchWindow {
    private var searchSinceMs: Int64?
    private var lastRosterFingerprint: String?
    private var lastUnlinkedOwnDevices = 0
    ...
    func observe(rosterFingerprint: String, unlinkedOwnDevices: Int, nowMs: Int64) {
        let shortfall = max(0, unlinkedOwnDevices)
        let rosterChanged = lastRosterFingerprint != rosterFingerprint
        ...
        searchSinceMs = coreLanOwnDeviceSearchSince(
            ...
            unlinkedOwnDevices: UInt32(shortfall),
            previousUnlinkedOwnDevices: UInt32(lastUnlinkedOwnDevices),
            nowMs: nowMs
        )
...
func ownRosterFingerprint(deviceIds: [Data]) -> String {
    deviceIds
        .map { id in id.map { String(format: "%02x", $0) }.joined() }
        .sorted()
        .joined(separator: ",")
}
