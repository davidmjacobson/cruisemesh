...
func friendImportFailureText(_ error: Error, text: String) -> String {
    ...
    // A `CMLINK1:` device-link offer is a code this build mints itself, so
    // "update the app" would be a lie. WP6 owns the words that belong here.
    if let core = error as? CoreError, case .DeviceLinkOffer = core {
