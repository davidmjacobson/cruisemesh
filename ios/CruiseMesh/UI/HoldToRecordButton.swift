...
struct HoldToRecordButton: View {
    ...
    var body: some View {
        ...
        .onChange(of: scenePhase) { phase in
            ...
            // screen. iOS has no `audio` background mode here (and should not
            // grow one for this), so the session is torn down under the running
            // recorder the moment the app backgrounds. Stop and say so rather
            // than let a locked recording auto-send whatever survived.
    ...
    private var holdControl: some View {
        ...
        .frame(maxWidth: isRecording ? CGFloat.infinity : nil)
